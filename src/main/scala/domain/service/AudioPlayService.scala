...
trait AudioPlayService[F[_]]:
  ...
  /** Create new audio play.
    ...
    *   audio play request
    * @return
    *   `Right(AudioPlay)` if success, `Left(AudioPlayError)` if fail
    * @note
    * user must have [[AudioPlayServicePermission.Write]] permission.
    */
  def create(
      user: User,
      ac: AudioPlayRequest
 ...
 *
  ...
  /** Deletes existing audio play.
    ...
    *   audio play id
    * @return
    *   `Right(Unit)` if success, `Left(AudioPlayError)` if fail
    * @note
    * user must have [[AudioPlayServicePermission.Write]] permission.
    */
  def delete(user: User, id: MediaResourceID): F[Either[AudioPlayServiceError, Unit]]
