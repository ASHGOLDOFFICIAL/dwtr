...
trait TranslationService[F[_]]:
  /** Find translation by given identity.
    ...
    * @return
    *   requested translation if found
    */
  ...
  /** Create new translation.
    ...
    *   ID of original
    * @return
    *   Right(Translation) if success, Left(TranslationError) if fail
    * @note
    *   user must have [[TranslationServicePermission.Create]] permission.
    */
  def create(
      user: User,
  ...
  /** Updates existing translation.
    ...
    *   `Right(Translation)` if success, `Left(TranslationError)` if fail
    * @note
    *   user must have [[TranslationServicePermission.Update]] permission.
    */
  def update(
      user: User,
  ...
  /** Deletes existing translation.
    ...
    *   translation identity
    * @return
    *   Right(Unit) if success, Left(TranslationError) if fail
    * @note
    *   user must have [[TranslationServicePermission.Delete]] permission.
    */
  def delete(
      user: User,
      id: TranslationIdentity
